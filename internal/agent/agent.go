...
func (a *Agent) Run(ctx context.Context, in RunInput, sink events.Sink) (RunOutput, error) {
	...
	if err != nil {
		return RunOutput{}, fmt.Errorf("load history: %w", err)
	}
	...
	toolNames := a.tools.AlwaysBoundNames()
	for _, m := range matches {
		...
		toolNames = append(toolNames, m.AllowedTools...)
	}

	// 4. Resolve tools (registry-backed plus this turn's AG-UI client tools)
	// and describe them for the prompt.
	clientTools, clientToolReturnDirectly, err := buildClientTools(in.ClientTools)
	if err != nil {
		return RunOutput{}, err
	}
	boundTools := append(a.tools.ByNames(toolNames), clientTools...)
	toolDesc := describeTools(a.tools, boundTools)
	...
	if err != nil {
		return RunOutput{}, fmt.Errorf("build chat model: %w", err)
	}

	// 7. Build and run the ReAct agent.
	reactAgent, err := buildReactAgent(ctx, cm, boundTools, systemPrompt, a.acfg.MaxSteps, clientToolReturnDirectly)
	if err != nil {
		return RunOutput{}, fmt.Errorf("build react agent: %w", err)
	}
	...
	asm.messageStart(messageID, in.Model, estIn)
	...
	if err := a.store.Messages.Append(persistCtx, &assistantMsg); err != nil {
	...
	a.recordRun(persistCtx, sess.ID, assistantMsg.ID, runStats, runErr)
...
func describeTools(reg *tools.Registry, bound []einotool.BaseTool) map[string]string {
	all := reg.Describe()
	out := map[string]string{}
	for _, t := range bound {
		if info, err := t.Info(context.Background()); err == nil {
			if d, ok := all[info.Name]; ok {
				out[info.Name] = d
			} else {
